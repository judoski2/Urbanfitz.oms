...
app.put('/api/orders/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (statusChanged) {
      sms.sendStatusSMS(order, newStatus).catch(e => console.error('SMS error:', e));
    }
    if (payChanged && !statusChanged) {
      sms.sendPaymentSMS(order, newPayStatus).catch(e => console.error('SMS error:', e));
    }
...
app.get('/api/orders/:id/proforma', requireAuth, async (req, res) => {
  try {
    ...
    doc.fillColor(BLUE).text('\u20A6'+Number(order.amount).toLocaleString(), totX, y, { width: 115, align: 'right' });
    y += 30;
...
// ── BULK PDF BY STATUS ────────────────────────────────────────────────────────
app.get('/api/orders/bulk-pdf', requireAuth, async (req, res) => {
  try {
    ...
    doc.font('Helvetica-Bold').fontSize(14).fillColor(BLACK).text('\u20A6' + totalAmt.toLocaleString(), 175, 83);

    doc.font('Helvetica-Bold').fontSize(8).fillColor(GRAY).text('PAID', 330, 73);
    doc.font('Helvetica-Bold').fontSize(14).fillColor('#059669').text('\u20A6' + paidAmt.toLocaleString(), 330, 83);

    doc.font('Helvetica-Bold').fontSize(8).fillColor(GRAY).text('OUTSTANDING', 440, 73);
    doc.font('Helvetica-Bold').fontSize(14).fillColor(RED).text('\u20A6' + unpaidAmt.toLocaleString(), 440, 83);
    ...
    orders.forEach((o, idx) => {
      ...
      doc.font('Helvetica-Bold').text('\u20A6' + Number(o.amount).toLocaleString(), 395, y+7);
    ...
    doc.text('\u20A6' + totalAmt.toLocaleString(), 395, y+7);
    y += 22;
    ...
    orders.forEach((o, idx) => {
      ...
      doc.font('Helvetica-Bold').fontSize(14).fillColor(BLACK).text('\u20A6' + Number(o.amount).toLocaleString(), 120, dy-3);
      dy += 20;
