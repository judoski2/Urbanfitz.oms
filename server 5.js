...
app.post('/api/orders', requireAuth, async (req, res) => {
  try {
    const { customer, phone, address, pay_status, status, notes, date, delivery_type, tracking_number, logistics, items } = req.body;
    if (!customer || !phone) return res.status(400).json({ error: 'Missing required fields' });

    // Support both single product (legacy) and multiple items
    let orderItems = [];
    if (items && Array.isArray(items) && items.length > 0) {
      orderItems = items;
    } else {
      // Legacy single product support
      const { product, qty, size, color, amount } = req.body;
      if (!product || !amount) return res.status(400).json({ error: 'Missing product details' });
      orderItems = [{ product, qty: qty||1, size: size||'', color: color||'', unit_price: amount }];
    }

    // Calculate total amount
    const totalAmount = orderItems.reduce((sum, item) => sum + (Number(item.unit_price) * Number(item.qty||1)), 0);

    const id = await nextOrderId();
    const orderDate = date || new Date().toISOString().split('T')[0];
    // Use first item for legacy product field
    ...
    await db.run(
      `INSERT INTO orders (id,customer,phone,address,product,qty,size,color,amount,pay_status,status,notes,date,delivery_type,tracking_number,logistics)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      id, customer, phone, address||'', firstItem.product, firstItem.qty||1,
      firstItem.size||'', firstItem.color||'', totalAmount,
      pay_status||'Unpaid', status||'New', notes||'', orderDate,
    ...
    for (const item of orderItems) {
      await db.run(
        `INSERT INTO order_items (order_id, product, qty, size, color, unit_price) VALUES (?,?,?,?,?,?)`,
        id, item.product, item.qty||1, item.size||'', item.color||'', item.unit_price
      );
    }

    const order = await db.get('SELECT * FROM orders WHERE id=?', id);
    const savedItems = await db.all('SELECT * FROM order_items WHERE order_id=? ORDER BY id', id);
...
app.get('/api/orders/:id/items', requireAuth, async (req, res) => {
  try {
    const items = await db.all('SELECT * FROM order_items WHERE order_id=? ORDER BY id', req.params.id);
    res.json(items);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/orders/:id', requireAuth, async (req, res) => {
  try {
    ...
    // Handle items (multi-product) if provided
    let orderItems = null;
    let totalAmount = existing.amount;
    if (items && Array.isArray(items) && items.length > 0) {
      orderItems = items;
      totalAmount = items.reduce((sum, item) => sum + (Number(item.unit_price) * Number(item.qty||1)), 0);
    } else {
      // Legacy single-product update support
      const { product, qty, size, color, amount } = req.body;
      if (product || amount) {
        totalAmount = amount || existing.amount;
      }
    }

    const firstItem = orderItems ? orderItems[0] : null;
    ...
    await db.run(
      ...
      firstItem?firstItem.product:(req.body.product||existing.product),
      firstItem?(firstItem.qty||1):(req.body.qty||existing.qty),
      firstItem?(firstItem.size||''):(req.body.size??existing.size),
      firstItem?(firstItem.color||''):(req.body.color??existing.color),
      totalAmount, newPayStatus, newStatus,
    ...
    // Replace order_items if multi-item data was sent
    if (orderItems) {
      await db.run('DELETE FROM order_items WHERE order_id=?', req.params.id);
      for (const item of orderItems) {
        await db.run(
          `INSERT INTO order_items (order_id, product, qty, size, color, unit_price) VALUES (?,?,?,?,?,?)`,
          req.params.id, item.product, item.qty||1, item.size||'', item.color||'', item.unit_price
        );
      }
    }

    const order = await db.get('SELECT * FROM orders WHERE id=?', req.params.id);
    const savedItems = await db.all('SELECT * FROM order_items WHERE order_id=? ORDER BY id', req.params.id);
    ...
    res.json({ ...order, items: savedItems, sms_sent: statusChanged || payChanged });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/analytics', requireAuth, async (req, res) => {
  try {
    ...
    const revenueByDay = await db.all(`
      SELECT
        TO_CHAR(DATE(date), 'DD Mon') as day,
        DATE(date) as day_sort,
        SUM(amount) as total_revenue,
        SUM(CASE WHEN pay_status='Paid' THEN amount ELSE 0 END) as paid_revenue,
        COUNT(*) as order_count
      FROM orders
      WHERE DATE(date) >= NOW() - INTERVAL '30 days'
      GROUP BY day, day_sort
      ORDER BY day_sort ASC
    `);

    res.json({ revenueByMonth, revenueByDay, topProducts, topCustomers, paymentBreakdown, statusBreakdown, totals });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/orders/:id/pdf', requireAuth, async (req, res) => {
  try {
    ...
    doc.text('PRODUCT', 55, y+7); doc.text('SIZE', 230, y+7); doc.text('COLOR', 285, y+7);
    doc.text('QTY', 360, y+7); doc.text('UNIT', 395, y+7); doc.text('AMOUNT', 450, y+7, { width: 95, align: 'right' });
    y += 20;

    // Fetch order items — fall back to single product if none
    const orderItems = await db.all('SELECT * FROM order_items WHERE order_id=? ORDER BY id', order.id);
    const itemsToShow = orderItems.length > 0 ? orderItems : [{
      product: order.product, size: order.size, color: order.color,
      qty: order.qty, unit_price: order.amount
    }];

    itemsToShow.forEach((item, idx) => {
      const rowBg = idx % 2 === 0 ? '#F9F9F9' : '#FFFFFF';
      const itemAmt = Number(item.unit_price) * Number(item.qty||1);
      doc.rect(50, y, 495, 22).fill(rowBg).stroke('#EEEEEE');
      doc.font('Helvetica').fontSize(8).fillColor(BLACK);
      doc.text(String(item.product).substring(0,22), 55, y+7, { width: 170 });
      doc.text(item.size||'-', 230, y+7, { width: 50 });
      doc.text(item.color||'-', 285, y+7, { width: 70 });
      doc.text(String(item.qty||1), 360, y+7, { width: 30 });
      doc.text('NGN ' + Number(item.unit_price).toLocaleString(), 395, y+7, { width: 50 });
      doc.font('Helvetica-Bold').text('NGN ' + itemAmt.toLocaleString(), 450, y+7, { width: 95, align: 'right' });
      y += 22;
    });
    y += 10;
    const totX = 380;
...
app.get('/api/orders/:id/proforma', requireAuth, async (req, res) => {
  try {
    ...
    doc.rect(50, y, 495, 20).fill(BLACK);
    doc.font('Helvetica-Bold').fontSize(7).fillColor('#FFFFFF');
    doc.text('PRODUCT', 55, y+7); doc.text('SIZE', 230, y+7);
    doc.text('COLOR', 285, y+7); doc.text('QTY', 360, y+7);
    doc.text('UNIT', 395, y+7); doc.text('AMOUNT', 450, y+7, { width: 95, align: 'right' });
    y += 20;

    const proformaItems = await db.all('SELECT * FROM order_items WHERE order_id=? ORDER BY id', order.id);
    const proformaToShow = proformaItems.length > 0 ? proformaItems : [{
      product: order.product, size: order.size, color: order.color,
      qty: order.qty, unit_price: order.amount
    }];

    proformaToShow.forEach((item, idx) => {
      const itemAmt = Number(item.unit_price) * Number(item.qty||1);
    ...
    doc.rect(50, y, 495, 52).fill('#E8F5E9').stroke('#A5D6A7');
    doc.rect(50, y, 4, 52).fill('#25D366');
    doc.font('Helvetica-Bold').fontSize(10).fillColor('#1B5E20')
       .text('📱 IMPORTANT — Payment Confirmation', 62, y+8);
    doc.font('Helvetica').fontSize(9).fillColor('#2E7D32')
       .text('Kindly forward your payment receipt to our WhatsApp: 07038245181', 62, y+22, { width: 470 });
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#1B5E20')
       .text('Payment is only confirmed upon receipt of proof of payment.', 62, y+34, { width: 470 });
    y += 62;
...
app.get('/api/products', requireAuth, async (req, res) => {
  try {
    const products = await db.all('SELECT * FROM products ORDER BY category ASC, name ASC');
    res.json(products);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/products', requireAuth, async (req, res) => {
  try {
    ...
    const result = await db.run(
      `INSERT INTO products (name, description, price, image_url, category) VALUES (?,?,?,?,?) RETURNING *`,
      name, description||'', price, image_url||'', category||''
    );
    const product = await db.get('SELECT * FROM products WHERE id=(SELECT MAX(id) FROM products)');
    res.json(product);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/products/:id', requireAuth, async (req, res) => {
  try {
    ...
    await db.run(
      `UPDATE products SET name=?,description=?,price=?,image_url=?,category=?,available=? WHERE id=?`,
      name, description||'', price, image_url||'', category||'',
      available !== false, req.params.id
    );
...
app.get('/api/cloudinary-signature', requireAuth, (req, res) => {
  ...
  if (!cloudName || !apiKey || !apiSecret) {
    return res.json({ configured: false, message: 'Cloudinary not configured' });
  }
  const timestamp = Math.round(Date.now() / 1000);
  const crypto = require('crypto');
  const str = `folder=urbanfitz-products&timestamp=${timestamp}${apiSecret}`;
  const signature = crypto.createHash('sha256').update(str).digest('hex');
  res.json({ configured: true, timestamp, signature, cloudName, apiKey, folder: 'urbanfitz-products' });
});
