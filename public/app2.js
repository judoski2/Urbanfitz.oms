  ...
  try {
    ...
    showSheet(`
      ...
      <div style="display:flex;gap:10px;margin-top:10px">
        ...
        <button onclick="shareProforma('${order.id}')" style="flex:1;background:linear-gradient(135deg,#1D6FE8,#1558C0);border:none;border-radius:12px;padding:13px;color:#fff;font-weight:800;font-size:14px;cursor:pointer">📋 Proforma</button>
      </div>
    `, order.id);
...
function orderFormHTML(order) {
  ...
  return `<div class="form-grid">
    ...
    <div class="form-group"><label>Amount (₦) *</label><input type="number" name="amount" value="${v('amount')}" placeholder="45000"/></div>
    <div class="form-group full"><label>Product *</label><input type="text" name="product" value="${v('product')}" placeholder="e.g. Custom Ankara Suit"/></div>
    ...
    <div class="form-group"><label>Quantity</label><input type="number" name="qty" value="${v('qty',1)}" min="1"/></div>
    <div class="form-group"><label>Size</label><input type="text" name="size" value="${v('size')}" placeholder="S/M/L/XL"/></div>
    <div class="form-group"><label>Color</label><input type="text" name="color" value="${v('color')}" placeholder="e.g. Blue/Gold"/></div>
...
async function submitOrderForm(id) {
  ...
  if (!form.customer || !form.phone || !form.product || !form.amount) { showToast('Fill all required fields (*)', true); return; }
  try {
