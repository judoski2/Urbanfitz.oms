...
function renderApp() {
  ...
  <div id="main-app" class="active">
    ...
    <nav class="bottom-nav">
      ...
      <button class="nav-btn" data-page="measurements" onclick="navigateTo('measurements')">
        <span class="nav-icon">📏</span><span>Measurements</span><div class="nav-dot"></div>
      </button>
      ...
      <button class="nav-btn" data-page="sms" onclick="navigateTo('sms')">
        <span class="nav-icon">💬</span><span>SMS</span><div class="nav-dot"></div>
      </button>
    </nav>
  </div>`;
}
...
function downloadProforma(id) {
  ...
  window.location.href = '/api/orders/' + id + '/proforma';
}
...
async function downloadBulkPDF(status, payStatus, dateFrom, dateTo, btn) {
  ...
  try {
    ...
    window.location.href = '/api/orders/bulk-pdf?' + params.toString();
    setTimeout(() => {
