      ...
      <div class="white-box">
        ...
        ${s.byStatus.map(r => {
          ...
          return `<div class="status-row">
            ...
            <span class="status-count">${r.c}</span>
          </div>`;
        }).join('')}
      </div>
      <div class="white-box">
  ...
  el.innerHTML = `
    ...
    <div id="filter-panel" style="display:${showFilters?'block':'none'}">
      <div class="filter-panel">
        ...
        <div class="chip-row">
          ${['All',...ALL_STATUSES].map(s => `<button class="chip ${filterStatus===s?'active':''}" onclick="setFStatus('${s}')">${s}</button>`).join('')}
        </div>
...
async function viewOrder(id) {
  ...
  showSheet(`
    ...
    <div class="detail-actions">
      ...
      <button class="btn-invoice" onclick="closeSheet();downloadInvoice('${order.id}')">🧾 PDF Invoice</button>
    </div>
  `, order.id);
}
